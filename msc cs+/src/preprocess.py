...
def detection(img):
    ...
    dets = detector(img, 0)  # 使用detector进行人脸检测 dets为返回的结果
...
def predictor(img, dets):
    ...
    new_img = img[landmark[:, 0], landmark[:, 1]]
...
# def test():
#     lena = cv.cvtColor(cv.imread('D:/tesr/train_set/images/0.jpg', flags=1), cv.COLOR_BGR2RGB)
#     print(lena.shape)
#
...
if __name__ == '__main__':
    img = cv.imread('D:/tesr/train_set/images/0.jpg', cv.IMREAD_GRAYSCALE)
    img, dets, shape_list, img_list, pt_pos_list = deal(img)
    print(shape_list)
